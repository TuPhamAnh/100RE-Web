 ...
 * Executive laboratory overview: active experiments, protocol checklists, equipment status, and Drive datasets.
 */
...
export async function renderDashboard(container) {
  container.innerHTML = `
    <div class="ws-loader-center">
      ...
      <p style="margin-top:12px;">Loading SciNote ELN Dashboard...</p>
    </div>
  `;
    ...
    container.innerHTML = `
      ...
      <div class="ws-page-header">
        ...
        <div class="ws-page-actions">
          ...
          <a href="#workflows" class="btn-ws-ghost"><i class="fa-solid fa-diagram-project"></i> Workflow Canvas</a>
        </div>
      </div>
      ...
      <div class="ws-stats-grid">
        <div class="ws-stat-card">
          ...
          <div class="ws-stat-content">
            ...
            <div class="ws-stat-label">Active Experiments</div>
          </div>
        </div>
        ...
        <div class="ws-stat-card">
          ...
          <div class="ws-stat-content">
            ...
            <div class="ws-stat-label">Protocol Tasks</div>
          </div>
        </div>
        ...
        <div class="ws-stat-card">
          ...
          <div class="ws-stat-content">
            ...
            <div class="ws-stat-label">Instruments Allocated</div>
          </div>
        </div>
      ...
      <div class="ws-dashboard-grid" style="display:grid; grid-template-columns:repeat(auto-fit, minmax(460px, 1fr)); gap:24px; margin-top:24px;">
        ...
        <div class="ws-card" style="margin-bottom:0;">
          <div class="ws-card-header" style="padding:16px 20px; border-bottom:1px solid var(--ws-border); display:flex; justify-content:space-between; align-items:center;">
            <h3 class="ws-card-title"><i class="fa-solid fa-flask" style="color:#0284c7;"></i> Ongoing Lab Experiments</h3>
            <a href="#experiments" class="ws-card-link">View All &rarr;</a>
          </div>
          <div style="padding:16px 20px; display:flex; flex-direction:column; gap:12px;">
            ${experiments.length === 0 ? '<p style="color:var(--ws-text-muted);">No active experiments.</p>' : experiments.slice(0, 3).map(exp => `
              <div style="padding:12px; background:#f8fafc; border-radius:6px; border:1px solid var(--ws-border); display:flex; justify-content:space-between; align-items:center;">
                <div>
                  ...
                  <div style="font-size:0.775rem; color:var(--ws-text-muted);">
                    <i class="fa-solid fa-diagram-project"></i> ${escapeHtml(exp.project?.name || 'General Project')}
                  </div>
                </div>
        ...
        <div class="ws-card" style="margin-bottom:0;">
          <div class="ws-card-header" style="padding:16px 20px; border-bottom:1px solid var(--ws-border); display:flex; justify-content:space-between; align-items:center;">
            <h3 class="ws-card-title"><i class="fa-solid fa-microchip" style="color:var(--ws-primary);"></i> Lab Hardware &amp; Testbeds</h3>
            <a href="#inventory" class="ws-card-link">Manage &rarr;</a>
          </div>
        ...
        <div class="ws-card" style="margin-bottom:0;">
          <div class="ws-card-header" style="padding:16px 20px; border-bottom:1px solid var(--ws-border); display:flex; justify-content:space-between; align-items:center;">
            <h3 class="ws-card-title"><i class="fa-solid fa-list-check" style="color:#d97706;"></i> My Assigned Protocol Tasks</h3>
            <a href="#tasks?filter=me" class="ws-card-link">All My Tasks &rarr;</a>
          </div>
          <div style="padding:16px 20px;">
            ${myTasks.length === 0 ? '<p style="color:var(--ws-text-muted); font-size:0.875rem;">No tasks assigned directly to you right now.</p>' : `
                ...
                ${myTasks.slice(0, 4).map(t => `
                  <div style="padding:10px 12px; background:#fff; border:1px solid var(--ws-border); border-radius:6px; display:flex; justify-content:space-between; align-items:center; cursor:pointer;" onclick="window.openTaskDetail('${t.id}')">
                    <div>
                      ...
                      <div style="font-size:0.75rem; color:var(--ws-text-muted); margin-top:2px;">Due: ${formatDate(t.due_date)}</div>
                    </div>
        ...
        <div class="ws-card" style="margin-bottom:0;">
          ...
          <div style="padding:16px 20px;">
            ${recentDatasets.length === 0 ? '<p style="color:var(--ws-text-muted); font-size:0.875rem;">No datasets uploaded yet.</p>' : `
              <div style="display:flex; flex-direction:column; gap:8px;">
                ${recentDatasets.slice(0, 3).map(ds => `
                  <div style="padding:10px 12px; background:#fff; border:1px solid var(--ws-border); border-radius:6px; display:flex; justify-content:space-between; align-items:center;">
                    <div>
                      ...
                      <div style="font-size:0.75rem; color:var(--ws-text-muted);">Format: <code>${escapeHtml(ds.format)}</code></div>
                    </div>
