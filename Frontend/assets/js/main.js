...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Impeccable 3D Interactive Parallax on Hero Title
  function initHeroTitle3DParallax() {
    const heroSection = document.querySelector('.page-hero');
    const heroTitle = document.querySelector('.home-hero-title');
    if (!heroSection || !heroTitle || window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
    ...
    heroSection.addEventListener('mousemove', (e) => {
      if (!ticking) {
        window.requestAnimationFrame(() => {
          ...
          heroTitle.style.transform = `perspective(900px) rotateY(${x * 12}deg) rotateX(${-y * 12}deg) translateZ(15px)`;
          ticking = false;
        });
        ticking = true;
      }
    ...
    heroSection.addEventListener('mouseleave', () => {
      heroTitle.style.transform = 'perspective(900px) rotateY(0deg) rotateX(0deg) translateZ(0px)';
    });
  }
